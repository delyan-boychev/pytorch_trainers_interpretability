...
def int_grads_compare_regular_robust(regular, robust, images, labels, classes, normalizer=None, treshold=0):
    ...
    fig = plt.figure(figsize=(20*num_cols, 70))
    ...
    for j, sub in enumerate(subfigs):
        ...
        for i, subfig in enumerate(sub2):
            ...
            grad = integrated_grad.random_baseline_integrated_grads(image, pr.argmax(dim=1), steps=50, num_random_trials=10, batch_size=50)
            grad2 = integrated_grad2.random_baseline_integrated_grads(image, pr2.argmax(dim=1), steps=50, num_random_trials=10, batch_size=50)
