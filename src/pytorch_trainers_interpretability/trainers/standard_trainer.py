...
class StandardTrainer:
    def __init__(self, model="resnet18", pretrained=False, criterion=nn.CrossEntropyLoss(),
        ...
        self.save_info = SaveInfo(
            save_path=self.save_path, resume_path=resume_path, adv_train=False)
        ...
        self.testloader = data.DataLoader(
            dataset=testset, batch_size=batch_size, shuffle=False, num_workers=2)
        if lr_scheduler is not None:
            ...
            if lr_scheduler is "OneCycleLR":
                self.scheduler = schedulers[lr_scheduler](
                    optimizer=self.optimizer, steps_per_epoch=len(self.trainloader), epochs=epochs)
            else:
                self.scheduler = schedulers[lr_scheduler](
