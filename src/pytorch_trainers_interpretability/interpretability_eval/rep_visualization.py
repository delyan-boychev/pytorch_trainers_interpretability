...
class RepVisualization:
    ...
    def rep_inversion(self, orig_images, source_images):
        ...
        _, r = self.model(self.normalizer(
            orig_images.cuda()), with_latent=True)
        xadv = self.att(source_images.clone(), r.clone(),
                        use_best=False, targeted=True)
        return xadv
    ...
    def set_testset_rep_vector(self, testloader):
        ...
        for i, (im, targ) in itr:
            ...
            with torch.no_grad():
                _, rep = self.model(self.normalizer(
                    im.cuda()), with_latent=True)
    ...
    def feature_vis_max_activation(self, actv, input_size):
        if self.rep_test is None or self.img_test is None:
            raise Exception(
                "You should first run set_testset_rep_vector to get maximal activations")
        img_source = 0.5 * torch.ones(input_size)
        vis = self.feature_vis(img_source, actv)
        def dist1(x): return x
        def dist2(x): return -x
        if type(actv) is list:
            def dist1(x): return np.squeeze(np.average(x, axis=2), axis=1)
            def dist2(x): return np.squeeze(np.average(-x, axis=2), axis=1)
